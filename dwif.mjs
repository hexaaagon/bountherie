...
export function getDefaultOutputName(inputPath) {
  ...
  if (parsed.ext.toLowerCase() === ".avif") {
    return `${parsed.name}-resized.avif`;
  }
  return `${parsed.name}-resized${parsed.ext || ".png"}`;
}

function isAvifPath(filePath) {
...
async function removeDirectoryWithRetries(targetPath, attempts = 6) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      if (!isRetriableCleanupError(error) || attempt === attempts - 1) {
        throw error;
      }
...
async function removeDirectoryBestEffort(targetPath, attempts = 12) {
  try {
    await removeDirectoryWithRetries(targetPath, attempts);
  } catch {
    // Sharp/ImageMagick can briefly hold temp files open on Windows.
    // Cleanup should not turn a successful export into a failed operation.
  }
}
...
async function processImageCore({
  inputPath,
  outputPath,
  manualTopStrip = null,
  manualRadius = null,
...
async function convertAvifToExperimentalWebp(inputPath, outputPath) {
  try {
    const source = sharp(inputPath, SHARP_SOURCE_OPTIONS);
    const metadata = await source.metadata();
    const frameCount = metadata.pages ?? 1;
    ...
    await sharp(inputPath, SHARP_STILL_SOURCE_OPTIONS)
      .webp({
        lossless: true,
        effort: 6
      })
...
async function convertExperimentalWebpToAvif(inputPath, outputPath) {
  await runProcess("magick", [
    inputPath,
    "-quality",
    "90",
...
async function processExperimentalAvif({
  inputPath,
  outputPath,
  manualTopStrip = null,
  manualRadius = null,
  ...
  const bridgeOutputPath = isAvifPath(outputPath)
    ? path.join(tempDir, `${path.parse(outputPath).name}-processed.webp`)
    : outputPath;
  const bridgeInputPath = isAvifPath(inputPath)
    ? path.join(tempDir, `${path.parse(inputPath).name}-source.webp`)
    : inputPath;

  try {
    if (isAvifPath(inputPath)) {
      onProgress?.({ current: 0, total: 1, stage: "avif-bridge", percent: 0 });
      await convertAvifToExperimentalWebp(inputPath, bridgeInputPath);
      onProgress?.({ current: 1, total: 1, stage: "avif-bridge", percent: 100 });
    }
    ...
    if (isAvifPath(outputPath)) {
      await convertExperimentalWebpToAvif(bridgeOutputPath, outputPath);
    }
    ...
    return {
      ...
      warning: [
        result.warning,
        (isAvifPath(inputPath) || isAvifPath(outputPath))
          ? "Experimental AVIF support used a temporary lossless WEBP bridge and finalized AVIF with ImageMagick."
          : null
      ].filter(Boolean).join(" ") || null
    };
  } catch (error) {
    throw new Error(`Experimental AVIF processing failed: ${error.message}`);
  } finally {
    await removeDirectoryBestEffort(tempDir);
  }
}

export async function processImage(options) {
  if (isAvifPath(options.inputPath) || isAvifPath(options.outputPath)) {
    return processExperimentalAvif(options);
  }

  return processImageCore(options);
}
